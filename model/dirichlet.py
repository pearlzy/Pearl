...
class MLE_Dirchlet(object):
        ...
        for quad in trainQuads:
            for entity in [quad[0], quad[2]]:
                if entity in entity_occ_times.keys():
                    if (quad[3], quad[4]) in entity_occ_times[entity].keys():
                        entity_occ_times[entity][(quad[3], quad[4])] += 1
                    else:
                        entity_occ_times[entity][(quad[3], quad[4])] = 1
    ...
    def get_relations_observed_data(self, trainQuads):
        ...
        for quad in trainQuads:
            ...
            for time1,time2 in occ_times.keys():
                if time1 >= quad[3] and time2 <= quad[4]:
                    continue
                observed[int((abs(quad[3] - time1) + abs(quad[4] - time2)) / 2) // self.timespan] = occ_times[(time1,time2)]
            ...
            for time1,time2 in occ_times.keys():
                if time1 >= quad[3] and time2 <= quad[4]:
                    continue
                reversed_r_observed[int((abs(quad[3] - time1) + abs(quad[4] - time2)) / 2) // self.timespan] = occ_times[
                    (time1,time2)]
