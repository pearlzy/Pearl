...
class PG(object):
    ...
    def get_reward(self, current_entites, cts1, cts2, answers_arr):
         ...
         reward = torch.zeros(current_entites.shape[0], dtype=torch.float32).to('cuda')

         for i in range(current_entites.shape[0]):
           condition_list = []
           condition1 = torch.any(cts1[i]+125726 == answers_arr[i])
           #todo
           condition2 = torch.any(cts2[i]+125726 == answers_arr[i])
           condition = torch.any(current_entites[i] == answers_arr[i])
           condition_list.append(condition | condition1 | condition2)
           reward[i] = torch.where(torch.any(torch.stack(condition_list), dim=0), positive[i], negative[i])
         return reward
